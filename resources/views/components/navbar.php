...
$isLoggedIn = Application::$app->session->get('user') !== null;
$userName = Application::$app->session->get('user_name');
$requestPath = parse_url($_SERVER['REQUEST_URI'],PHP_URL_PATH);
$isHome = in_array($requestPath,['/','/index.php']);
$subdomain = Application::$app->getSubdomain();
$isSubdomain = $subdomain && !in_array($subdomain, ['blogify', 'www']);
...
<?php if($isSubdomain && !$isLoggedIn): ?>
<a href="/login" class="nav-link">Sign In</a>
<a href="#" class="nav-link btn">Subscribe</a>
<?php elseif($showNav): ?>
<a href="<?= $isLoggedIn ? '/dashboard' : '/login' ?>" class="nav-link">Sign In</a>
<a href="/register" class="nav-link btn">Get Started</a>
<?php endif; ?>
</div>
</nav>
</div>
